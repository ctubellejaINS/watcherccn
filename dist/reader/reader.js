...
import { autoInsertdDocumentMngt } from "../db/documentMngt.js";
dotenv.config();
const path = process.env.READER_PATH;
export const readerPath = async () => {
    if (!path) {
...
export const readLoopContent = async (files) => {
    const folderName = moment().subtract(1, "d").format("YYYY-MM-DD");
    for (const file of files) {
        ...
        if (!checkDirExists) {
            saveLogs(`INFO`, `NO FOLDER FOUND in ${ccnPath}`, new Date().toISOString());
        }
        else {
            ...
            for (const dirFile of dirFiles) {
                ...
                await autoInsertdDocumentMngt(r);
                //   save to database
            }
        }
    }
